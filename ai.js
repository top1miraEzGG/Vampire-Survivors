...
var GameAI = (function() {
    ...
    function shootAtNearestEnemy() {
        ...
        // Сохраняем угол оружия для рендеринга
        ...
        if (GameConfig.VISUAL_CONFIG.particleEffects && typeof GameParticles !== 'undefined') {
            GameParticles.createShotParticles(player.x + 10, player.y + 10, angle);
        }
        ...
        for (var i = enemies.length - 1; i >= 0; i--) {
            ...
            for (var j = bullets.length - 1; j >= 0; j--) {
                ...
                if (checkCollision(b, e)) {
                    ...
                    if (GameConfig.VISUAL_CONFIG.particleEffects && typeof GameParticles !== 'undefined') {
                        GameParticles.createHitParticles(e.posX + 10, e.posY + 10);
                    }
                    
                    GameState.removeBullet(j);
                    
                    if (e.health <= 0) {
                        // Создаем частицы смерти врага
                        if (GameConfig.VISUAL_CONFIG.particleEffects && typeof GameParticles !== 'undefined') {
                            GameParticles.createDeathParticles(e.posX + 10, e.posY + 10);
                        }
                        
                        // Начисляем опыт за убийство
                        GameState.addExperience(GameState.getExpForKill());
