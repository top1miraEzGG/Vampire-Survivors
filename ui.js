...
var GameUI = (function() {
    ...
    function init() {
        ...
        elements.expIndicator = document.getElementById('exp-indicator');
        elements.levelIndicator = document.getElementById('level-indicator');
        elements.levelUpMessage = document.getElementById('level-up-message');
    ...
    function updateHealth() {
        ...
        elements.playerHpValue.textContent = Math.ceil(player.health) + '/' + player.maxHealth;
    ...
    function updateExperience() {
        if (!elements.expIndicator) return;
        
        var player = GameState.player();
        var pct = (player.currentExp / player.expToNextLevel) * 100;
        elements.expIndicator.style.width = pct + '%';
        
        if (elements.levelIndicator) {
            elements.levelIndicator.textContent = 'Уровень: ' + player.level;
        }
    }
    
    function updateLevelUp(newLevel) {
        if (elements.levelUpMessage) {
            elements.levelUpMessage.textContent = 'Уровень ' + newLevel + '!';
    ...
    if (elements.expIndicator) elements.expIndicator.parentElement.style.display = 'flex';
    if (elements.levelIndicator) elements.levelIndicator.style.display = 'block';
